...
def test(model, device, test_loader, learningConfig, retWGAcc = False):
    model.eval()
    test_lossEpoch = 0
    correct = 0
    countOfDataProcessed = 0
    ...
    # Log group-specific accuracies
    group_accuracies_dict = {}
    for group_id, group_data in group_accuracy.items():
        group_correct = group_data['correct']
        group_total = group_data['total']
        group_accuracy_percentage = 100. * group_correct / group_total
        group_accuracies_dict[group_id] = group_accuracy_percentage
        print(f'Group {group_id}: Accuracy: {group_correct}/{group_total} ({group_accuracy_percentage:.2f}%)')
        wandb.log({f"Test Accuracy for group {group_id} Per Epoch": group_accuracy_percentage})

    if retWGAcc: #This is if want to get Worst group accuracy returned too
        wgaAcc = min(group_accuracies_dict.values())
        return accuracyTest, wgaAcc
    else:
        return accuracyTest
...
def train_hyperparameter_search(model, device, train_loader, optimizer, epoch, learningConfig, display=True):
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        '''
        Want half of the batches for training for hyperparameter search and other half for test function
        '''
...
def test_hyperparameter_search(model, device, test_loader, learningConfig, retWGAcc = False):
    model.eval()
    test_lossEpoch = 0
    correct = 0
    countOfDataProcessed = 0
