...
from HAM10000Data.data_transforms import AugWaterbirdsCelebATransform
...
class HAM10000Dataset(Dataset):
    def __init__(self, basedir, split="train", transform=None):
        ...
        #in metadata file for ham10000, when split is val, still test folder used (index for split are 0, 1, 2)
        if split == "val":
            folderFile = ("test" + "/ham_metadata.csv")

        else:
            folderFile = (split + "/ham_metadata.csv")
            
        metadata_df = pd.read_csv(os.path.join(basedir, folderFile))
...
def get_transform_ham(target_resolution, train, augment_data, custom_data_transform):
...
def get_loader_ham(data, train, reweight_groups, reweight_classes, reweight_places, **kwargs):
